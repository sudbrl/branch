...
def process_uploaded_file(file_url):
    """
    Fetches the Excel file from a URL using requests (handles servers
    that reject plain urllib/pandas requests) and processes it.
    """
    try:
        ...
        # Apply scoring rules (vectorized)
        ...
        return None, "Connection error — could not reach the data URL."
    except requests.exceptions.Timeout:
        return None, "Request timed out while fetching data."
    except Exception as e:
        return None, str(e)
...
if check_password():
    ...
    # Resolve data URL from secrets or fall back to hardcoded URL
        ...
        with tab1:
            ...
            with col_right:
                ...
                fig_bar.update_layout(
                    ...
                    xaxis_title='Risk Score', yaxis_title='Branch Code'
                )
        ...
        with tab2:
            ...
            with col1:
                st.markdown(
                    ...
                    f"border-left:4px solid {color_scheme['primary']};text-align:center;'>"
                    f"<b>Total Risk Score</b>"
                    f"<div style='font-size:2.5rem;'>{branch_data['Total Score']:.2f}</div></div>",
                    unsafe_allow_html=True
                )
            with col2:
                st.markdown(
                    ...
                    f"border-left:4px solid {color_scheme['primary']};text-align:center;'>"
                    f"<b>Risk Grade</b>"
                    f"<div style='font-size:2.5rem;color:{color_scheme['primary']}'>{grade}</div></div>",
                    unsafe_allow_html=True
                )
            with col3:
                ...
                st.markdown(
                    f"<div style='background:white;padding:1.5rem;border-radius:12px;"
                    f"border-left:4px solid {color_scheme['primary']};text-align:center;'>"
                    f"<b>Percentile Rank</b>"
                    f"<div style='font-size:2.5rem;'>{percentile:.0f}%</div></div>",
                    unsafe_allow_html=True
                )
            with col4:
                ...
                st.markdown(
                    ...
                    f"border-left:4px solid {color_scheme['primary']};text-align:center;'>"
                    f"<b>Risk Status</b>"
                    f"<div style='margin-top:0.5rem;font-weight:bold;'>{status_text}</div></div>",
                    unsafe_allow_html=True
                )
            ...
            with col_breakdown:
                ...
                param_data = [
                    {
                        'Parameter': p,
                        'Actual': format_value(branch_data.get(p, 'N/A'), p),
                        'Score': f"{s:.2f}"
                    }
        ...
        with tab3:
            ...
            if selected_attr:
                ...
                if pd.api.types.is_numeric_dtype(df[selected_attr]):
                    ...
                    filtered_attr_df = df[
                        (df[selected_attr] >= min_input) & (df[selected_attr] <= max_input)
                    ]
                else:
                    unique_vals   = df[selected_attr].astype(str).unique().tolist()
                    selected_vals = st.multiselect(
                        f"Select Values for {selected_attr}", unique_vals, default=unique_vals
                    )
                    if selected_vals:
                ...
                if not filtered_attr_df.empty:
                    ...
                    st.dataframe(
                        filtered_attr_df.style.format(attr_format_dict),
